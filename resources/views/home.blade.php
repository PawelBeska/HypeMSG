...
@section('content')
    ...
    <script>
        ...
        function check() {
           if($("input#to").val()) {
               $.ajax({
                   url: "{{route('home.user.get.messages')}}",
                   type: 'POST',
                   data: {'id': $("input#to").val(),'_token':"{{csrf_token()}}"},
                   cache: false,
                   global: false,
        ...
        function sendMessage(data)
        {
            console.log(data);
            $.ajax({
                url: "{{route('home.user.send.message')}}",
                type: 'POST',
                data: data,
                cache: false,
                global: false,
        ...
        const Message = ({id,to,from,message,created_at}) => `
                        <div class="message-item  ${from=={{auth()->id()}} ? 'outgoing-message' : null} " id="${id}">
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('li.list-group-item').click(function(e) {
                    ...
                    $("input#to").val(id);
                    $.ajax({
                        url: "{{route('home.user.chat.header')}}",
                        type: 'POST',
